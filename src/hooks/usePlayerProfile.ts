...
export const usePlayerProfile = (gameId: GameID) => {
  ...
  // NOTE: This assumes your gameStatsSlice holds stats for one game at a time.
  // We'll rename `ayoStats` to `stats` for clarity.
  const { ayoStats: stats, loading: statsLoading, error: statsError } = useAppSelector((state) => state.gameStats);
